...
def average_log_error(pred, lbl, eps=1e-6):
    pred = pred.detach().cpu().numpy()
    lbl = lbl.detach().cpu().numpy()

    pred_clamped = np.clip(pred, eps, None)
    lbl_clamped = np.clip(lbl, eps, None)
    
    return np.mean(np.abs(np.log(pred_clamped) - np.log(lbl_clamped)))
...
def edge_preservation_index(pred, target):
    pred_edges = sobel_edges(pred)
    target_edges = sobel_edges(target)

    numerator = (pred_edges * target_edges).sum(dim=[1, 2, 3])
    denom = torch.sqrt((pred_edges ** 2).sum(dim=[1, 2, 3]) * (target_edges ** 2).sum(dim=[1, 2, 3]) + 1e-8)
